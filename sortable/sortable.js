/*
  ...
  version 2
  7th April 2007
  Stuart Langridge, http://www.kryogenix.org/code/browser/sortable/
...
sortable = {
  init: function() {
    ...
    sortable.DATE_RE = /^(\d\d?)[\/\.-](\d\d?)[\/\.-]((\d\d)?\d\d)$/;
    
    forEach(document.getElementsByTagName('table'), function(table) {
      if (table.className.search(/\bsortable\b/) != -1) {
        sortable.makeSortable(table);
      }
			
			// remove sortable_sorted classes
			if (table.tHead) {
				theadrow = table.tHead.rows[0].cells[0].parentNode;
			...
			if (table.tHead) {
				table.tHead.rows[0].cells[0].className += ' sortable_sorted';
				sortfwdind = document.createElement('span');
				sortfwdind.id = "sortable_sortfwdind";
				sortfwdind.innerHTML = stIsIE ? '&nbsp<font face="webdings">6</font>' : '&nbsp;&#x25BE;';
				table.tHead.rows[0].cells[0].appendChild(sortfwdind);
			}
			...
			if (table.tHead) {
				row_array = [];
				col = table.tHead.rows[0].cells[0].sortable_columnindex;
				rows = table.tHead.rows[0].cells[0].sortable_tbody.rows;
				...
				row_array.sort(table.tHead.rows[0].cells[0].sortable_sortfunction);
				
				tb = table.tHead.rows[0].cells[0].sortable_tbody;
  ...
  makeSortable: function(table) {
    ...
    for (var i=0; i<headrow.length; i++) {
      ...
      if (!headrow[i].className.match(/\bsortable_nosort\b/)) { // skip this col
        mtch = headrow[i].className.match(/\bsortable_([a-z0-9]+)\b/);
        if (mtch) { override = mtch[1]; }
	      if (mtch && typeof sortable["sort_"+override] == 'function') {
	        headrow[i].sortable_sortfunction = sortable["sort_"+override];
	      } else {
	        headrow[i].sortable_sortfunction = sortable.guessType(table,i);
	      }
  ...
  guessType: function(table, column) {
    ...
    for (var i=0; i<table.tBodies[0].rows.length; i++) {
      text = sortable.getInnerText(table.tBodies[0].rows[i].cells[column]);
      if (text != '') {
        if (text.match(/^-?[£$¤]?[\d,.]+%?$/)) {
          return sortable.sort_numeric;
        }
        // check for a date: dd/mm/yyyy or dd/mm/yy 
        // can have / or . or - as separator
        // can be mm/dd as well
        possdate = text.match(sortable.DATE_RE)
        if (possdate) {
          // looks like a date
          ...
          if (first > 12) {
            // definitely dd/mm
            return sortable.sort_ddmm;
          } else if (second > 12) {
            return sortable.sort_mmdd;
          } else {
  ...
  sort_ddmm: function(a,b) {
    mtch = a[0].match(sortable.DATE_RE);
    y = mtch[3]; m = mtch[2]; d = mtch[1];
    if (m.length == 1) m = '0'+m;
    if (d.length == 1) d = '0'+d;
    dt1 = y+m+d;
    mtch = b[0].match(sortable.DATE_RE);
    y = mtch[3]; m = mtch[2]; d = mtch[1];
    if (m.length == 1) m = '0'+m;
    if (d.length == 1) d = '0'+d;
    dt2 = y+m+d;
...
if (document.addEventListener) {
    document.addEventListener("DOMContentLoaded", sortable.init, false);
}
...
/*@if (@_win32)
    ...
    script.onreadystatechange = function() {
        if (this.readyState == "complete") {
            sortable.init(); // call the onload handler
        }
    };
/*@end @*/
